...
game::game()
{
   cout << "\nGame Start! Below is how the squares are numbered.\n";
   for(int i = 0; i < 9; i++){state[i] = (i + 49);}
   disp();
   for(int i = 0; i < 9; i++){state[i] = ' ';}
   turn = 0;
}

char game::getState(int indx){return state[indx];}
...
{  
   ...
   {
      cout << "Err: square already filled\n";
      return 2;
   }
